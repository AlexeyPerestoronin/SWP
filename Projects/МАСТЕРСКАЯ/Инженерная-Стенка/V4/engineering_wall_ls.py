...
def prepare_steel_manufacturing_doc(ctx):
    ...
    execute = True
    if execute:
        steel_manufacturing_doc_folder = DOC_FOLDER / 'Steel' / 'Manufacturing'
        ...
        steel_6mm.prepare_data([f"{PROJECT_NAME} .+"],
                               step=True,
                               dxf=False,
...
@utils.sw_task(doc_string=f"Make complex documentation for '{PROJECT_NAME}'", pre=[clear_doc_folder, parse_saving_groups, prepare_steel_manufacturing_doc, convert_doc_to_zip])
def make_doc(ctx):
    saving_groups = getattr(ctx, 'saving_groups')
