...
QUANTITY_EVALUATOR = lambda x: x * 16
PROJECT_PATH = pathlib.Path(__file__).with_name(f'{PROJECT_NAME}.SLDPRT')
DOC_FOLDER = PROJECT_PATH.with_name(f'{PROJECT_NAME} Стойка-Удлинительная DOC-x{QUANTITY_EVALUATOR(1)}')
...
def prepare_manufacturing_doc(ctx):
    ...
    if execute:
        ...
        steel_4mm = utils.doc_creator.LaserCuttingManufacturingElementsTable(saving_groups)
        steel_4mm.prepare_data([f"{PROJECT_NAME} стойка-удлинительная су-.+"], step=True, dxf=True, save_folder_opt=manufacturing_doc_folder, quantity_evaluator=QUANTITY_EVALUATOR)

        utils.doc_creator.CNCLaserCuttingDocCreator(f"Стойка-Удлинительная для {PROJECT_NAME}") \
            .add_4mm_steel_sheet_table(steel_4mm) \
    ...
    for saving_group in saving_groups:
        if saving_group.mark is None:
            ...
            unused_elements = [
                ...
                f"{PROJECT_NAME} кронштейн-для-ВСС к-всс-рж",
                f"{PROJECT_NAME} кронштейн-для-ВСС к-всс-платформа-крепёжная",
                f"{PROJECT_NAME} кронштейн-для-ВСС к-всс-платформа-регулировочная",
