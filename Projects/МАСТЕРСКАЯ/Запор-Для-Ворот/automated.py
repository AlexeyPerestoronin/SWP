...
def make_doc(ctx):
    ...
    ../DOC
        ...
        file-N.step
    """
    project_path = pathlib.Path(__file__).with_name('Запор-Для-Ворот.SLDPRT')
    ...
    if execute:
        ...
        for (reference_body, quantity, reference_component, step_path) in save_paths_and_bodies:
            ...
            # steel_sheet_8
            if try_detect(r".+ПШ-.+") or try_detect(r".+ПП-.+"):
                utils.info.log_line(f"detected DOC-not-taken-into-account step-file: {step_path}")
            elif try_detect(r".+"):
                steel_sheet_8.append([component_full_name, quantity * 2])
            else:
