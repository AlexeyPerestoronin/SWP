...
PROJECT_NAME = 'Направляющий-Держатель-Для-Перфоратора'
...
def prepare_metal_manufacturing_doc(ctx):
    ...
    aluminum_block.prepare_data([f"{PROJECT_NAME} часть-\d"], step=True, dxf=False, save_folder_opt=doc_folder)
    aluminum_block_material_info = utils.doc_creator.MaterialInfoTable()
    aluminum_block_material_info.prepare_data([
        ["материал", "алюминий"],
        ["минимальный размер заготовки для фрезерования", "параллелограмм 180x80x45мм"],
    ])
    ...
    assert utils.prepare_archive(root_dir=doc_folder,
                                 ...
                                 archive_name=f"ТЗ (ТОЛЬКО ОЦЕНИТЬ) на производство металлических деталей для направляющего держателя для перфоратора",
                                 archive_type='zip',
                                 add_date=True)
